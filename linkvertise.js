...
// @resource logo.png https://bypass.city/generated/android-chrome-36x36.png
...
/* 
release: 12.6.2 (fccaadf), 1/19/2024, 5:25:01 AM
...
(() => {
  ...
  var UserScript = {
    ...
    getResourceURL: (name, fallbackUrl, isBlobUrl) => {
      return typeof GM_getResourceURL !== "undefined" ? new Promise((resolve) => resolve(GM_getResourceURL(name, isBlobUrl))) : typeof GM.getResourceURL !== "undefined" ? GM.getResourceURL(name, isBlobUrl) : new Promise((resolve) => resolve(fallbackUrl));
    },
  ...
  var config = {
    ...
    buildTime: "1/19/2024, 5:25:01 AM",
    branch: "release",
    release: "fccaadf",
    installed: true,
    releaseTag: "12.6.2 (fccaadf)"
  };
...
@keyframes glow {
  ...
  var linkvertiseListener = async () => {
    ...
    if (bypassData) {
      ...
      if (window.location.href.includes("linkvertise.com/48193/")) {
        notify({
          title: "KRNL DETECTED!",
          text: "Please wait 13 seconds to avoid getting blacklisted!",
          hideLinks: true
        });
        await sleep(13e3);
      }
  ...
  // src/main.ts
  (async () => {
    console.info("Linkvertise Userscript", config.releaseTag, window.location.href);
